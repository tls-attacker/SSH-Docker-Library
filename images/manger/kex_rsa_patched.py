...
class _KexRSA(Kex):
    ...
    def start(self):
        ...
        if self._conn.is_server():
            ...
            # Implement manger vulnerability part 1: Use static transient public key
            # self._trans_key = generate_private_key('ssh-rsa', self._key_size)
            self._trans_key = vulnerability.get_static_transient_public_key(
                self._key_size
            )
    ...
    def _process_secret(self, _pkttype, _pktid, packet):
        ...
        # Prints encrypted secret as integers
        # print("Encrypted secret: ")
        # print([x for x in self._encrypted_k])
        packet.check_end()

        # Implement manger vulnerability part 2: Different error message on starting byte not 00
        if not vulnerability.first_byte_is_zero(self._trans_key, self._encrypted_k):
            raise KeyExchangeFailed(
                "Key exchange decryption failed because first byte is not 00"
            )
...
for _name, _hash_alg, _key_size, _hash_size, _default in (
    ...
    register_kex_alg(_name, _KexRSA, _hash_alg, (_key_size, _hash_size), _default)
