...
if __name__ == '__main__':
    ...
    # Add argument to specify the test class to run
    parser.add_argument('--test-class', type=str, choices=['AbsolutTelnetSSHClientTest', 'TeraTermSSHClientTest'],
                        required=True, help='Test class to run')

    # Add specific arguments for different test cases
    parser.add_argument('--port-range', type=str,
                        help='Port range in the format start_port-end_port (e.g., 3000-3005) for AbsolutTelnet .tnt file generation')
    ...
    function_mapping = {
        'AbsolutTelnetSSHClientTest': AbsolutTelnetSSHClientTest,
        'TeraTermSSHClientTest': TeraTermSSHClientTest
    }
    ...
    # Select the test class based on user input
    selected_test_class = function_mapping.get(args.test_class)
    methodname = method_mapping.get(selected_test_class)

    start_time = time.time()

    if selected_test_class == AbsolutTelnetSSHClientTest and methodname == "test_build_profiles":
        ...
        for port in range(start_port, end_port + 1):
            test_class_name, result = run_tests(selected_test_class, methodname, hostname, username, port, args.debug)
            if result.wasSuccessful():
                print(f"All repetitions of test suite '{test_class_name}' passed on port {port}")
            else:
                print(f"Some repetitions of test suite '{test_class_name}' failed on port {port}")
        ...
        test_class_name, result = run_tests(selected_test_class, methodname, hostname, username, args.port, args.debug)
        if result.wasSuccessful():
